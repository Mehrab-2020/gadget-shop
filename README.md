...
- **Bluetooth Speakers**: Portable speakers with powerful sound
...
2. Drag and drop these files:
   ...
   - `script.js`
   - `README.md`
3. Create a new folder called `images` by:
   - Click "Add file" → "Create new file"
   - Type `images/placeholder.txt` in the name field
   - Commit this file
   - Then upload all product images from the `images` folder to this new folder
...
# Add all files (including the images folder)
git add .
...
To add images for more products:
...
2. **Upload images**:
   ...
   - Use descriptive filenames (e.g., `samsung-charger-25w.webp`)

3. **Update the product in `script.js`**:
   ```javascript
   {
       id: 1,
       name: "Your Product Name",
       category: "chargers",  // or "tws", "earphones", "speakers"
       price: 1500,
       icon: "⚡",  // Fallback emoji if image doesn't load
       image: "images/your-product-image.webp",  // Add this line
       description: "Product description",
       badge: "New"
   }
   ```

Products without an `image` property will display the emoji icon as a fallback.
